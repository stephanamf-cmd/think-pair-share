...
export function keywordEdges(
  ...
  opts: { minShared: number; ignoreText?: string; maxShare?: number },
  ...
  for (const [s, list] of byWord) {
    for (let a = 0; a < list.length; a++)
      for (let b = a + 1; b < list.length; b++) {
        ...
        (shared.get(key) ?? shared.set(key, []).get(key)!).push(surface.get(s) ?? s);
      }
  }
  ...
  for (const [key, words] of shared) {
    ...
    edges.push({ source: kw[a].id, target: kw[b].id, words });
  }
